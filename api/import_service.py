...
def build_search_text(entry: dict[str, Any]) -> str:
    content = entry.get("content") or entry.get("official_text", "")
    sections = [
...
def normalize_entry(entry: dict[str, Any]) -> dict[str, Any]:
    content = entry.get("content") or entry.get("official_text", "")
    section = entry.get("section") or entry.get("bylaw_number")
    topic = entry.get("topic") or entry.get("chapter", "")
    law_name = entry.get("law_name", "Maharashtra Cooperative Housing Society Model Bye-laws")
    return {
        "law_name": law_name,
        "section": str(section or ""),
        "subsection": entry.get("subsection", "") or "",
        "title": entry.get("title") or entry.get("full_code") or f"Bye-law {section}",
        "topic": topic or "model bye-laws",
...
def load_dataset(dataset_path: str | None = None) -> list[dict[str, Any]]:
    ...
    if path.exists():
        if path.suffix.lower() == ".json":
            return [normalize_entry(entry) for entry in json.loads(path.read_text(encoding="utf-8"))]
